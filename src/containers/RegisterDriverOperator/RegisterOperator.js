...
const RegisterOperator = () => {
    ...
    const getOperator = async () => {
        try {
            ...
            const allOper = response.data.data.users.filter(Operator =>
                Operator.userType === 'Operator');
            setGetOp(allOper)
            console.log("operators",allOper)
    ...
    const registerOperator = async (e) => {
        ...
        if (fullname.trim().length == '') return Notify('Username field should not be empty', 'error');
                ...
                <div className="bg-white w-full  mp:w-8/12  md:w-6/12  xl:w-4/12 2xl:w-3/12 rounded-lg p-4 pb-8">
                    ...
                    <div className="card-body">
                        <form onSubmit={registerOperator} action="/Operators" className=' sp:px-8 mp:px-5 sm:px-10  md:px-8 lg:px-12'>
                            ...
                            <div className="input my-3 h-9 ">
                                <div className="grouped-input bg-secondary-40 flex items-center  h-full w-full rounded-md">
                                    <input type="text" name="fullname" className=" bg-transparent border-0 outline-none px-5 font-sans text-xs text-secondary-50 h-5 w-4/5" value={fullname} placeholder="User name" onChange={e => setUsername(e.target.value)} />
                                </div>
                            </div>
                            <div className="input my-3 h-9 ">
                                <div className="grouped-input bg-secondary-40 flex items-center h-full w-full rounded-md">
